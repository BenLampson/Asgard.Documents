...
- When invoking `build/deploy-asgard-docs.sh` from Windows PowerShell, strip carriage returns before piping the script to SSH (for example, `(Get-Content -Raw build/deploy-asgard-docs.sh).Replace("`r", "") | ssh ...`). Otherwise Bash can report a false failure after Nginx has already switched and reloaded the release.
...
- If an authoritative source repository is private, keep its canonical repository URL in the product registry for authorized maintainers, record public `404`/authentication responses as release diagnostics, and label the access boundary in maintainer notes. Do not replace the URL with a guessed mirror or suppress the warning as if the repository were public.
